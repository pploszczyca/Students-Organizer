...
class AssignmentRoutes[F[_]: JsonDecoder: Sync](
...
) extends (AuthMiddleware[F, StudentEntity] => HttpRoutes[F]),Http4sDsl[F]:
  ...
  override def apply(authMiddleware: AuthMiddleware[F, StudentEntity]): HttpRoutes[F] =
    Router(
